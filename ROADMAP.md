...
- [x] Full app restyle to docs/DESIGN.md via 4-agent fan-out: design system primitives (PageHeader gradient bands, StatPill, EmptyState, BrightEarsLogo component, LEAD_STATUS_META single source), pipeline (stat pills, colored column headers, hover-lift cards, friendly empty states + whole-pipeline CTA), lead detail (chat-bubble thread, "envelope" draft card), calendar (today ring, deep-teal gig chips, weekend tints), packages (price-hero cards), settings (emoji section tiles, elevated Pro plan, mono lead-address pill), onboarding (palette-walking progress chips, gradient backdrop, removed dark-block violation). Presentation-only — build + 55 tests green; verified visually signed-in; deployed to staging. AWAITING FOUNDER VERDICT — his eyes are the acceptance test
