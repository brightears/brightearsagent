...
- [x] ADR-001: **Postmark** (inbound-parse quality + deliverability; provider-agnostic InboundEmail type isolates a future swap) — production outbound is live. `replies@mail.brightears.io` passed SPF + DKIM alignment and mail-tester scored both reply shapes 10/10 on 2026-07-30.
...
- [x] Clerk auth + tenant resolution — production Clerk is live; `lib/tenant.ts` `getCurrentBusiness()` remains the single tenant boundary.
...
- [x] Outbound send: transport abstraction — dev transport writes `.eml` files; production Postmark transport is active from `replies@mail.brightears.io` with white-label From and owner Reply-To.
...
- [x] Acceptance: fixture → webhook → draft PENDING → approve → Postmark delivery → inbox is live-verified. A real-device push tap-through remains a product polish check, not an email launch blocker.
...
- [x] Stripe — production catalog, webhook, portal and live subscription are active. Test-mode fixtures remain local only; the founder's tenant has a live STARTER subscription.
...
- [ ] Acceptance: beta stranger-test (signup → onboard → first drafted reply < 20 min, no help). Clerk and live Stripe are configured; this now needs a real beta tester, not more infrastructure.

## Design-polish pass (founder feedback June 11) — ✅ done (June 11, 2026)
- [x] Full app restyle to docs/DESIGN.md via 4-agent fan-out: design system primitives (PageHeader gradient bands, StatPill, EmptyState, BrightEarsLogo component, LEAD_STATUS_META single source), pipeline, lead detail, calendar, packages, settings and onboarding. Presentation-only; now deployed in production.
...
- [ ] Acceptance remainder: rerun production Lighthouse against `brightears.io` and target ≥90. Canonicals and JSON-LD now correctly point at the live apex.
...
- [x] GitHub: repo `brightears/brightearsagent`, pushed via existing keychain creds (no new token needed); the chat-exposed `Bright Ears Agent` Render API key was revoked on 2026-07-30 and no replacement was stored.
...
- [x] Historical staging Lighthouse baseline: SEO 100 · A11y 96 · Perf 78 · BP 73, TBT 0ms, CLS 0.007. A fresh production run remains open above.
- [x] Release gates (2026-07-30): Next.js patched to 16.2.12; CI runs TypeScript, ESLint, 655 unit tests and the production build; Render deploys only after CI passes.
- [x] DB backup + recovery drill (2026-07-30): Render exposes Point-in-Time Recovery for the previous 7 days and logical exports retained at least 7 days. The 22:08:06 GMT+7 recovery point restored into an isolated Postgres 16 Basic-256mb database; read-only checks found all 34 completed migrations and non-empty core tables. The scratch database was deleted after verification without touching production.
- [x] Cron hardening (2026-07-30): all four live commands use masked `CRON_SECRET` at runtime, Bearer auth, non-2xx failure, and a 120-second timeout. No command contains `?secret=`.
- [x] Delivery-state recovery (2026-07-30): bounce/complaint/auto-reply state, corrected-address recovery, sequence reopening, reporting exclusions, migration and UI deployed. Production migration `20260730144500_postmark_delivery_state` applied.
- [x] Postmark delivery webhook (2026-07-30): outbound stream subscribed to Bounce + Spam Complaint only, HTTP Basic authentication configured from the masked Render secret, provider read-back verified, and authenticated production probes returned 200 for both event classes without changing production records.
- [x] Independent uptime monitor (2026-07-30): UptimeRobot checks `https://brightears.io/api/health` every 5 minutes, emails the account's existing alert contact, and reported the first check Up at 279 ms. The onboarding test notification was accepted; no public status page was created.
- [x] Founder-approved Render credential rotation (2026-07-30): `CRON_SECRET` regenerated across the web service and four cron jobs and the 23:00 scheduled sequence run returned 200; deploy hook regenerated; dormant `Bright Ears Agent` API key revoked without creating or storing a replacement.
- [x] 7-day green soak completed; production health and all four cron heartbeats were green on 2026-07-30.
- [x] Production configuration: `brightears.io`, production Postmark, Clerk and live Stripe are active. This is no longer a staging deployment.

## Phase 8 — brightears.io cutover (completed; do not rerun)
- [x] 1. Vinyl remains on `agency.brightears.io` and is out of scope for this repository.
- [x] 2. Vinyl integrations were moved to the agency subdomain.
- [x] 3. The agency subdomain completed its soak before the apex move.
- [x] 4. `brightears.io` now serves this SaaS app; the agency service remains separate.
- [x] 5. Set up `in.brightears.io` (inbound parse MX) + `mail.brightears.io` (outbound SPF/DKIM/DMARC); switch outbound from sandbox — ✅ DONE, **verified end to end 2026-07-30**. Inbound: `*.in.brightears.io` MX → `inbound.postmarkapp.com` is a **wildcard**, so every future tenant's `leads@{slug}.in.` resolves with no per-tenant DNS work. Outbound: `OUTBOUND_FROM=replies@mail.brightears.io`, custom Return-Path `pm-bounces.mail.brightears.io` → `pm.mtasv.net`, and the **delivered** headers of a loopback probe show `Received-SPF: pass` plus `DKIM-Signature d=mail.brightears.io s=20260723080904pm` — so SPF and DKIM both pass and both align (DKIM exactly, so strict alignment holds too). DMARC is `p=none`. Standing check: `npx tsx scripts/preflight-live.ts`. NOTE: a DKIM selector is `<yyyymmddHHMMSS>pm` and cannot be brute-forced — read it off a delivered message.
  - [x] 5b. ✅ **DONE 2026-07-30 — mail-tester 10/10 ("Perfect, you can send") on BOTH client-facing shapes**, sent through the real `sendEmail()` with representative product copy (`scripts/mail-tester.ts <address> [reply|followup]`). First reply: 10/10. Follow-up carrying the `complianceFooter` opt-out link: also 10/10 — no URL or unsubscribe rule fired. SpamAssassin `-0.1/5.0`; the only rules hit were `SPF_PASS`, `SPF_HELO_PASS`, `DKIM_VALID`, `DKIM_VALID_AU` (author domain) and a Mailspike allow-listing. Third-party confirmation of `dkim=pass header.d=mail.brightears.io header.s=20260723080904pm`.
    - Two things this surfaced, neither blocking: (a) Postmark's shared transactional pool spans IPs of differing reputation — the two sends left on `50.31.205.206` (Mailspike H5 "excellent") and `104.245.209.246` (H2 "average"). Normal for a shared pool; only worth revisiting at volume. (b) OpenDMARC reported `dmarc=none (p=none dis=none)` — that is the **policy**, not a failure: SPF and DKIM both pass and align. The apex now publishes `v=spf1 include:_spf.google.com include:spf.mtasv.net ~all`; tightening DMARC still requires reviewing aggregate reports because the apex also carries Vinyl's mail.
- [ ] 6. Tell the 6 venues (one LINE broadcast via Vinyl — founder approves wording)
- [x] Acceptance: Bright Ears is live on `brightears.io`; Vinyl remains live on its separate agency service. Never use this roadmap as authorization to touch that service.
...
## Phase 10 — The Sales Agent: venue-led discovery v1 (ADR-004 as amended; probe: docs/PROBE-CITY-DEMAND.md)
- [x] 10.0 **City probe (THE GATE)** ✅ June 12: Austin/Nashville/Manchester — public posted demand ~0-1/wk everywhere (scraping public posts CUT); venue signals consistently real (~6-7 hot venues/60d/metro + standing-venue long tail). Founder confirmed pivot: venue prospecting = PRIMARY engine; marketplace alerts = secondary (membership-fed, existing parser). Conversion (venues replying) = the remaining bet → beta gate 10.9
- [x] 10.1 ✅ June 12, amended July 31: Profile/EPK expansion shipped — artist-profile fields on Business (matching: genres, event types, service cities, travel policy, fee floor + sweet spot, pitch languages, insured; pitching: headline, bio, optional video links, photo URLs, review quotes, notable venues); `lib/profile/strength.ts` = weighted strength meter + the pitch-readiness gate (one clear photo, bio, headline, ≥1 gig, genres, cities, fee floor; packages and video are not gates); `/dashboard/profile` editor; white-label hosted EPK at `/epk/[slug]` (404 unless epkEnabled; zero Bright Ears/AI mention; mailto CTA). v1 deferrals: service radius (named cities only), calendar-visited signal (gig count is the proxy)
- [x] 10.2 ✅ June 12 (live-verified: 43 real venues Manchester+Leeds, ~$0.01-0.02/tenant/day, cron crn-d8lrfh8js32c73b65it0 daily 05:00 UTC) — Venue engine — discovery: Venue + VenueSignal models; per-metro signal scanners (new-opening roundups/local news via search API 🔑, new Maps-class listings, venue hiring posts) + standing-venue indexing (bars/hotels/event spaces matching act+geo+fee profile); freshness/heat scoring
- [x] 10.2c ✅ June 12 — Opportunity TEMPERATURE model: HOT (opening — deciding now) / WARM (existing venue that already buys entertainment, not posting a need) / SEED (relationship-planting: one polite intro, NO follow-ups; 180-day re-touch engine deferred — data stored only). WARM query battery (≤8/metro) on an every-3rd-scan wheel (Business.discoveryScanCount); deterministic timing classifier `lib/venues/timing.ts` (HOT 60-90 by freshness / WARM 20-40 +calendar+hiring / SEED 5-15) — FIT and TIMING kept separate; per-temperature pitch templates (HOT date-ask / WARM rotation-intro / SEED 60-90-word file-me-away) + deterministic SEED no-follow-up guard; daily pitch-creation caps HOT 10 / WARM 5 / SEED 3 in tenant tz (`lib/outreach/caps.ts`); LinkedIn-named contacts stored as URL only (find-only law); feed: temperature chips, "% short-term" timing line, honest evidence lead on WARM/SEED cards, HOT-first ordering, SEED behind ?hunt=all
- [x] 10.3 ✅ June 12 (sending parked for 10.5; grounding rule added after live fabrication catch) — Venue engine — contact + pitch: published booking/events contact discovery (venue site/Google Business/press ONLY; LinkedIn = find-only handoff card); pitch drafting in artist voice + EPK link, venue-type + culture/language aware; jurisdiction rules engine (recipient-country: US/UK/TH standard, CA consent-mode, DE strict); caps 10-20/day, suppression, dedup, C&D kill switch
- [x] 10.4 ✅ June 12 — Opportunity feed: one home feed, venue cards + inbound cards with source badges ("I found this venue" / "Came to you"); fit score + ≤3 plain reasons + 1 caution; Approve/Edit/Skip (Skip asks why → tunes matching); morning digest incl. skipped-list; FOUND→PITCHED→(reply)→existing pipeline statuses
- [x] 10.5 ✅ June 13 (Gmail OAuth LIVE on prod; adversarial review + 13 fixes; CASA review deferred to public launch) — Own-mailbox sending: Gmail/Microsoft OAuth onboarding step (easy guided flow), SPF/DKIM/DMARC check before agent may send, recipient-local send windows
- [ ] 10.6 Marketplace-alert secondary intake: "plug in your memberships" — GigSalad/Bark/Bash lead-alert emails through the existing parser, tagged as posted-demand opportunities in the same feed
...
- [ ] DEFERRED founder gates (pre-launch): beta email list · Sentry decision · lawyer review of legal pages · EU/UK Art 27 reps (only if EU traction grows) · EU/UK B2C VAT (accountant, before EU scaling). On 2026-07-31 the founder rejected marketplace-style media gates: performance video is an optional EPK enhancement, and one clear performance photo is enough to unlock venue pitching; three photos remain the recommended profile-strength target. The live founder profile has four photos, including an ABar action shot of Norbert DJing, plus a confirmed 2025-12-31 Hilton Pattaya gig. Render account 2FA was enabled and live-verified on 2026-07-31; Google OAuth data-access verification was submitted and moved to under review the same day.
- [x] OPTIONAL polish (audit-flagged) — closed by the July 2026 build loop: post-checkout plan-state banner + at-cap banner rework (P1.4/P1.8/P4.1), lead-pack strike stands (C3) · venue reply tracking REPLIED/IN_CONVERSATION (P8.3), discovery cron codified in `scripts/render-crons.py`, step-5 ~90s calm poll fallback (C2).
...
## July 2026 build loop — phases P0–P14 DONE (July 7, 2026, branch `build/july-2026`)
The full record lives in **`docs/BUILD-JULY-2026.md`** (build queue for the audit in `docs/PRODUCT-AUDIT-JULY-2026.md`) — every `[x]` there carries its commit hash; not duplicated here. In one line: day-one activation, Gmail-forwarding unblock, coverage rotation, notify-by-default, billing edges, honesty/marketing sweep, ops hardening, the agent acting on its own (nightly auto-drafted pitches, venue reply capture, auto-send behind a 15-min cancel buffer), mobile habit pass, trust machinery, pre-Gate-1 money slice, every-artist engine, Studio roster (rosterCap 1/1/1/10), security mediums. The remaining founder input is the exact beta email list; Google OAuth data-access verification is externally under review.
