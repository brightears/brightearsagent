...
## Phase 10 — The Sales Agent: proactive discovery v1 (ADR-004; research: docs/RESEARCH-PROACTIVE-AGENT.md)
- [ ] 10.0 **City probe (THE GATE)**: 7-day fitting-opportunity volume for a representative performer profile in 2–3 metros — posted demand (free channels) + venue-prospecting signals. ≥~10/wk/metro combined → build; thin → re-scope promise (marketplace-speed + venue-led)
- [ ] 10.1 Profile/EPK expansion: matching fields (genres/vibe tags, event types, service radius/cities, travel policy, fee floor + sweet spot, languages, insured), pitching fields (headline, short bio, video links, photos, review quotes), auto-generated hosted one-page EPK (white-label, zero AI mention); "hunting license" quality gate — agent may FIND with 4 fields, may not PITCH until video+photos+bio+packages+calendar
- [ ] 10.2 Opportunity model + feed: one home feed, inbound + found cards with source badge ("Came to you" / "I found this"); fit score + ≤3 plain reasons + 1 caution; Approve/Edit/Skip (Skip asks one-tap why → tunes matching); morning digest incl. the skipped-list (trust engine); FOUND→PITCHED→(reply)→existing pipeline statuses
- [ ] 10.3 Intake pipes v1: alert-email intake (Craigslist saved searches, marketplace lead alerts, FB group notification emails) through the existing parser; open-web scan via search API (founder gate 🔑: Serper or similar account) with per-tier cadence daily/3-hourly
- [ ] 10.4 Venue-prospecting engine: new-opening signals (local news/press, new Maps listings, venue hiring posts, new venue socials) + operating-venue matching; published booking/events contact discovery (venue site/Google Business/press only — NO LinkedIn automation, LinkedIn = handoff card); pitch + EPK in artist's voice
- [ ] 10.5 Channel-class enforcement in code (auto-eligible / approve-gated / handoff-only) + jurisdiction rules engine keyed on recipient country (US/UK/TH standard, CA consent-mode, DE strict-mode) extending the compliance-footer system; suppression list, cross-channel dedup, 10–20/day caps, C&D same-day kill switch
- [ ] 10.6 Own-mailbox sending: Gmail/Microsoft OAuth onboarding step (easy guided flow), SPF/DKIM/DMARC verification gate before agent may send, send-windows recipient-local
- [ ] 10.7 Control plane: Telegram bot (digest, Approve/Edit/Skip buttons, deep links, one-time-token account linking) + web push; WhatsApp utility-template digest phase 2 of this phase
- [ ] 10.8 Multilingual + culturally-tuned pitch templates (recipient-language drafting); pitch-quality eval harness (extend the 16-scenario suite with venue-pitch + posted-reply scenarios per language)
- [ ] 10.9 Beta gates: instrument reply-rate per channel from day 1; ≥30% of beta artists get ≥1 real conversation from an agent-found opportunity within 14 days before charging; north star = % with agent-sourced booking inside 90 days
