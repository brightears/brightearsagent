...
- [x] DB backup verification (2026-07-30): Render exposes Point-in-Time Recovery for the previous 7 days and logical exports retained at least 7 days. A restore drill against a temporary database remains open.
...
- [x] 10.1 ✅ June 12: Profile/EPK expansion shipped — artist-profile fields on Business (matching: genres, event types, service cities, travel policy, fee floor + sweet spot, pitch languages, insured; pitching: headline, bio, video links, photo URLs, review quotes, notable venues); `lib/profile/strength.ts` = weighted strength meter + the hunting license (video ≥1, photos ≥3, bio, headline, ≥1 active package, ≥1 gig, genres, cities, fee floor); `/dashboard/profile` editor; white-label hosted EPK at `/epk/[slug]` (404 unless epkEnabled; zero Bright Ears/AI mention; mailto CTA). v1 deferrals: photo uploads (external URLs only), service radius (named cities only), calendar-visited signal (gig count is the proxy)
...
- [ ] DEFERRED founder gates (pre-launch): rotate exposed Render/cron secrets · profile assets (2 photos, video, calendar gig) · beta email list · Google OAuth verification-video submission · temporary DB restore drill · Sentry decision · lawyer review of legal pages · EU/UK Art 27 reps (only if EU traction grows) · EU/UK B2C VAT (accountant, before EU scaling).
