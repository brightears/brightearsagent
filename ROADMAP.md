...
- [ ] Register the Postmark Bounce + Spam Complaint webhook and run its provider-side test — endpoint is live and fail-closed; blocked only on a founder-authenticated Postmark browser session.
...
- [ ] DEFERRED founder gates (pre-launch): rotate exposed Render/cron secrets · profile assets (2 photos, video, calendar gig) · beta email list · Google OAuth verification video · temporary DB restore drill · Sentry decision · lawyer review of legal pages · EU/UK Art 27 reps (only if EU traction grows) · EU/UK B2C VAT (accountant, before EU scaling).
