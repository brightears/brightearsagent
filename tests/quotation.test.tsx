...
const business: BusinessForQuote = {
  ...
  feeFloor: 1_500_000, feeSweetSpot: 2_000_000, residencyRate: 800_000, gigTypes: ["one-off", "residency"],
...
describe("renderQuotationForLead", () => {
  ...
  it("renders an estimate when no package matches", async () => {
    const buf = await renderQuotationForLead({ ...lead, eventType: "corporate gala" }, { ...business, gigTypes: ["one-off"] }, NOW);
    expect(buf!.subarray(0, 5).toString()).toBe("%PDF-");
  });
  it("returns null when there's no pricing to quote from", async () => {
    const buf = await renderQuotationForLead(
      { ...lead, eventType: "gala" },
      { ...business, feeFloor: null, feeSweetSpot: null, residencyRate: null, packages: [], gigTypes: ["one-off"] },
      NOW,
    );
    expect(buf).toBeNull();
  });
