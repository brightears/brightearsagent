...
export function buildVoicePrompt(business: BusinessProfile, packages: PackageInfo[]): string {
  ...
  return [
    ...
    `8. LANGUAGE: reply in the language used in the client's latest message. For follow-ups, keep the language already used in the thread. Do not translate names, venue names, links or quoted package names.`,
    business.bookingLinkUrl
      ? `9. CLOSING LINK: when the client shows real interest or asks how to book (and the date is NOT conflicted), include this exact link as the easy next step to lock it in: ${business.bookingLinkUrl} — present it naturally (\"you can lock in your date here\"), never more than once per email.`
      : ``,
  ].join("\n\n");
}
