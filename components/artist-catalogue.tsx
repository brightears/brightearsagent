...
export function ArtistCatalogue({artists,th}:{artists:AgencyArtist[];th:boolean}){
 const [query,setQuery]=useState(""); const [genre,setGenre]=useState("all"); const [shortlist,setShortlist]=useState<string[]>([]);
 ...
 function toggle(id:string){setShortlist(items=>items.includes(id)?items.filter(i=>i!==id):items.length<6?[...items,id]:items);}
 return <>
