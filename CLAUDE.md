...
**Every tier = the complete assistant** (tier recut per `docs/ADR-003-scope-vs-price.md`): instant AI replies in the owner's voice, follow-up sequences until booked-or-dead, weekly report, spam filtering and approve-from-phone are in ALL tiers — gates are **leads volume, performers, auto-send autonomy and team seats**, never capability. 14-day free trial (full Pro, no card) → **Starter $25/mo** (15 leads/mo, 1 performer, owner approves every send) → **Pro $79/mo** (60 leads/mo, per-source auto-send autopilot) → **Studio $149/mo** (150 leads/mo, multi-performer routing, team seats). Guarantee: **REMOVED pending founder decision (audit A1, 2026-06-14)** — the "pays for itself in your first season, full refund" wording was an unenforceable, unverifiable claim (FTC/DMCC/ACL exposure) and has been pulled from all customer surfaces and JSON-LD. Interim live wording is the true risk-reversal only (free trial · no card · cancel anytime · pause-at-cap); the final refund policy is the founder's call. Overage: lead packs ($10 per 10 leads) — never surprise bills; pause + upsell instead **(note: lead-pack purchase is advertised but NOT yet built — audit C3 founder item)**. **v1.1 money path** (quote → contract-lite e-sign → deposit via the DJ's own payment link, zero custody) ships only after Gate 1 passes — scope locked in ADR-003.
