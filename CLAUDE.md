...
**Every tier = the complete assistant** (tier recut per `docs/ADR-003-scope-vs-price.md`): AI replies in the owner's voice, follow-up sequences until booked-or-dead, weekly report, spam filtering, approve-from-phone, AND the proactive **Hunt** (venue-finding agent) are in ALL tiers — gates are **inbound leads volume, performers, auto-send autonomy and team seats**, never capability. **NO free trial (founder decision 2026-06-14): subscribe to activate, backed by a 30-day money-back guarantee** (single-sourced in `lib/marketing/guarantee.ts`; honored operationally via Stripe refund). New tenants may onboard/set up for free, but the live agent (drafting replies, generating/sending venue pitches) requires an active paid subscription — represented app-side as plan=TRIAL + `trialEndsAt` in the past (`lib/tenant.ts`), which the metering layer reads as "agent paused" (`lib/billing/metering.ts`). **Starter $25/mo** (15 inbound leads/mo, 1 performer, owner approves every send) → **Pro $79/mo** (60 inbound leads/mo, per-source auto-send autopilot) → **Studio $149/mo** (150 inbound leads/mo, multi-performer routing, team seats). **Two SEPARATE allowances:** reactive inbound **leads** (metered per plan, above) and proactive **venue pitches** (shared across plans, gated by profile quality not tier — daily caps in `lib/outreach/caps.ts`: HOT 10 / WARM 5 / SEED 3, ≤18/day total). Overage: lead packs ($10 per 10 leads) — never surprise bills; pause + upsell instead **(note: lead-pack purchase is advertised but NOT yet built — audit C3 founder item)**. **v1.1 money path** (quote → contract-lite e-sign → deposit via the DJ's own payment link, zero custody) ships only after Gate 1 passes — scope locked in ADR-003.
