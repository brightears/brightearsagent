...
description: Use when asked to review a plan, challenge a proposal, run a CEO review, poke holes in an approach, think bigger about scope, or decide whether to expand or reduce the plan. Reviews plans, never implements.
---

<!-- Ported from garrytan/gstack @ 11de390b (openclaw/skills/gstack-openclaw-ceo-review/SKILL.md), MIT.
     Modifications: summary saves to docs/ (repo convention) instead of memory/; Step 0B names this
     repo's canonical docs. See ../README-gstack-port.md. -->
...
1. What existing code already partially or fully solves each sub-problem? Map every sub-problem to existing code. In this repo, also check the plan against `CLAUDE.md`, `docs/PRODUCT-BRIEF.md`, and `ROADMAP.md` — a plan that contradicts a locked founder decision is a P0 finding.
...
If the review produced material decisions, save the summary into the plan doc itself or as a dated file under `docs/` for future reference.
