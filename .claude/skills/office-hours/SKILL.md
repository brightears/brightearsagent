---
name: office-hours
description: Use when asked to brainstorm, evaluate whether an idea is worth building, run office hours, or think through a new product idea or design direction before any code is written. Produces a design doc, never code.
---

<!-- Ported from garrytan/gstack @ 11de390b (openclaw/skills/gstack-openclaw-office-hours/SKILL.md), MIT.
     Modifications: saves design docs to docs/ (repo convention) instead of memory/; Phase 1 names this
     repo's canonical docs; removed the closing YC application pitch. See ../README-gstack-port.md. -->
...
1. Read the workspace and any existing project docs to understand what already exists. In this repo the canonical context is `CLAUDE.md`, `docs/PRODUCT-BRIEF.md`, `ROADMAP.md`, and any `docs/*.md` touching the topic at hand.
...
Write the design document and save it to `docs/` in this repo, following the existing naming convention (e.g. `docs/TOPIC-MONTH-YEAR.md`), so future sessions can reference it.
