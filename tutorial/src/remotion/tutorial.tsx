...
const BrandMark: React.FC = () => (
  <div style={{display: "flex", alignItems: "center", gap: 16}}>
    <Img src={staticFile("brand/logo.svg")} style={{width: 58, height: 58}} />
    <div style={{fontFamily: "Avenir Next, Arial, sans-serif", fontWeight: 800, fontSize: 35, letterSpacing: -1.2, color: colors.bright}}>Bright Ears</div>
  </div>
);
...
const SceneCard: React.FC<{scene: RenderScene; index: number; total: number}> = ({scene, index, total}) => {
  ...
  return (
    <AbsoluteFill style={{background: colors.night, color: colors.bright, overflow: "hidden", fontFamily: "Avenir Next, Arial, sans-serif"}}>
      <div style={{position: "absolute", width: 820, height: 820, borderRadius: "50%", background: "linear-gradient(135deg, #ff2dae 15%, #ff8a00 85%)", opacity: 0.13, right: -260, top: -360}} />
      <div style={{position: "absolute", width: 560, height: 560, borderRadius: "50%", background: colors.cyan, opacity: 0.11, left: -210, bottom: -300}} />
      <div style={{position: "absolute", inset: "72px 92px", display: "flex", flexDirection: "column"}}>
        <div style={{display: "flex", justifyContent: "space-between", alignItems: "center"}}>
          <BrandMark />
          <div style={{fontSize: 24, letterSpacing: 1.5, color: "rgba(232,228,220,.7)"}}>TUTORIAL · {String(index + 1).padStart(2, "0")}/{String(total).padStart(2, "0")}</div>
        </div>
        <div style={{flex: 1, display: "flex", alignItems: "center", transform: `translateY(${lift}px)`, opacity}}>
          <div style={{maxWidth: 1370}}>
            <div style={{fontWeight: 800, color: colors.cyan, letterSpacing: 4, fontSize: 26, marginBottom: 26}}>{scene.eyebrow ?? "SAFE · REPEATABLE · REVIEWABLE"}</div>
                ...
                {scene.bullets.map((bullet, bulletIndex) => {
                  ...
                  return (
                    <div key={bullet} style={{display: "flex", gap: 18, fontSize: 34, lineHeight: 1.28, opacity: bulletProgress * exit, transform: `translateX(${interpolate(bulletProgress, [0, 1], [28, 0])}px)`}}>
                      <span style={{color: colors.magenta, fontWeight: 900}}>●</span><span>{bullet}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div style={{height: 7, borderRadius: 8, background: "rgba(232,228,220,.12)", overflow: "hidden"}}>
          <div style={{height: "100%", width: `${Math.min(100, Math.max(0, frame / scene.durationInFrames * 100))}%`, background: colors.cyan}} />
        </div>
      </div>
    </AbsoluteFill>
  );
  ...
  return (
    <AbsoluteFill style={{background: colors.night, opacity: fade, fontFamily: "Avenir Next, Arial, sans-serif"}}>
      ...
      <div style={{
        ...
        padding: "18px 22px",
        color: colors.bright,
        background: "rgba(23,22,31,.92)",
        border: "1px solid rgba(0,187,228,.38)",
        boxShadow: "0 14px 40px rgba(0,0,0,.24)",
      }}>
        <div style={{display: "flex", justifyContent: "space-between", gap: 18, fontSize: 17, fontWeight: 800, letterSpacing: 2.1, color: colors.cyan}}>
