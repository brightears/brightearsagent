...
Adapters: Thumbtack Partner API (apply early — only marketplace officially sanctioning automated replies), Bark via official Zapier (budget caps), The Knot/WW deep-link nudge preserves their Quick Responder badge, GigSalad draft+deep-link only. Own-mailbox send (the *proactive* sales agent, ADR-004) is Google-only: the artist's own Gmail via native minimal-scope OAuth (`gmail.send` only). The integration and founder account were live-tested, but public Google data-access verification was submitted on 2026-07-31 and was last known to be under review; current console status still needs founder reauthentication and read-back. Do not claim arbitrary-user Gmail onboarding is verified until that external gate clears. No Microsoft/Outlook OAuth is built. Phase 2: FB Messenger/Instagram via Meta review.
...
**Subscribe to activate (founder decision 2026-06-16, SUPERSEDES the 2026-06-14 "14-day free trial" decision): Starter/Pro/Studio monthly, cancel anytime; NO automatic free trial; NO money-back guarantee.** The product wording is single-sourced in `lib/marketing/guarantee.ts`. `/terms` contains the intended cancellation and billing language, but the public legal set is still marked draft, noindex and not effective; it cannot be treated as the operative customer contract until the founder and qualified counsel approve it, set an effective date and remove the shared banner.
