...
**Invited-beta exception (founder decision 2026-08-21; supersedes the Stripe-promotion mechanism in the preceding paragraph):** an exact `BETA_COMP_EMAILS` match against Clerk's verified primary email receives one non-renewing 30-day Starter entitlement on first tenant provisioning. It creates no Stripe customer, Checkout session, payment method or subscription. The stored plan remains `TRIAL`; the paired `betaStartedAt`/`trialEndsAt` timestamps activate Starter gates and caps until the exact end timestamp, then the agent pauses while all customer data remains saved. Ordinary signups remain subscribe-to-activate, and public marketing does not advertise this controlled beta.
