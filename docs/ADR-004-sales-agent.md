...
- **Sends from the artist's own Gmail** via native minimal-scope Google OAuth. Never from Postmark/shared infrastructure — one artist's cold pitches must never endanger the reactive product's deliverability (Postmark ToS requires permission-based mail). The Google-only implementation and founder account were live-tested; public data-access verification was submitted on 2026-07-31 and was last known to be under review, so arbitrary-user Gmail onboarding remains an external launch gate. Microsoft/Outlook OAuth is not built.
- **Hard caps:** ≤18 pitch creations and sends per local day per artist (HOT 10 / WARM 5 / SEED 3), recipient-local send windows, tenant suppression plus a product-wide hard stop for explicit opt-outs/C&D/complaints/definitive invalid recipients, repliers exit sequences instantly, and same-day kill on any cease-and-desist. The global hard-stop migration is pending production deployment. General cross-tenant frequency control for an otherwise-unsuppressed venue remains a monitored beta/legal decision, not a shipped claim.
- **Jurisdiction rules engine** (extends the existing per-country compliance footers): cold B2B email is jurisdiction-dependent — US/TH standard mode (identity + opt-out), **GB/Canada consent-mode**, **Germany/Austria strict-mode**, and unknown countries consent-mode/fail-closed. Rules are keyed on the *recipient's* country. Consent/strict messages are review-and-copy handoffs only and require confirmation of consent or another lawful basis; manual transport does not create one. Quality gatekeeper both directions: junk-gig filter on inbound opportunities; fit filter (act/geo/fee-floor) on outbound prospects.
- **Language safety before global automation:** English pitches may use the validated send path. Non-English pitches remain manual-review/copy-only handoffs and are covered by the release eval suite; culturally validated automatic multilingual sending is not yet shipped. Country expansion still requires the approved privacy, recipient-class and jurisdiction gates.
