...
export async function GET(req: Request) {
  ...
  // SERVER MISCONFIG (distinct from "unavailable"): OAuth configured but the
  // token-encryption key is missing/malformed. verifyState() HMACs on that key
  // and encryptToken() needs it — both would 500. Bounce with the DISTINCT
  // config reason rather than the "feature off" wording.
  if (!isTokenCryptoConfigured()) {
    return settingsRedirect(req, "?mailbox=error&reason=config");
  }
  if (!code || !state) {
  ...
  // Everything below is key-dependent (verifyState HMAC / exchange / encrypt /
  // upsert). Wrap it so an UNEXPECTED throw still redirects to settings with an
  // error param instead of surfacing a raw 500. NEVER log tokens/secrets/codes.
  try {
    // CSRF: the state from Google must match the signed cookie AND verify
    // against this tenant — neither alone is enough.
    ...
    // Unexpected failure in the key-dependent section — honest config error, no
    // 500, no secret/token/code in any log.
    return settingsRedirect(req, "?mailbox=error&reason=config");
  }
}
